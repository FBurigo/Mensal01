...
def run_test(nome, method, url, body=None, esperado=200):
    ...
    log(
        f"Status esperado: {esperado} | Status obtido: {codigo} | Duracao: {duracao_ms}ms"
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Testa a API Biblioteca Pessoal de ponta a ponta."
    )
    parser.add_argument(
        "--base-url",
    ...
    sufixo = datetime.now().strftime(
        "%Y%m%d%H%M%S"
    )  # evita colisão de ISBN entre execuções

    # 1. Infra
    results.append(
        run_test("Swagger (/api/docs)", "GET", f"{base}/api/docs", esperado=200)
    )
    results.append(
        run_test(
            "Health check (/api/health)", "GET", f"{base}/api/health", esperado=200
        )
    )
    ...
    r_criar = run_test(
        "Criar livro (POST)", "POST", f"{base}/api/books", body=novo_livro, esperado=201
    )
    ...
    results.append(
        run_test(
            "Listar livros (GET /api/books)", "GET", f"{base}/api/books", esperado=200
        )
    )
    results.append(
        run_test(
            "Consultar livro criado (GET /api/books/{id})",
            "GET",
            f"{base}/api/books/{livro_id}",
            esperado=200,
        )
    )
    ...
    results.append(
        run_test(
            "Substituir livro (PUT)",
            "PUT",
            f"{base}/api/books/{livro_id}",
            body=livro_substituido,
            esperado=200,
        )
    )
    ...
    results.append(
        run_test(
            ...
            "PATCH",
            f"{base}/api/books/{livro_id}/status",
            body={"reading_status": "LENDO"},
            esperado=200,
        )
    )
    ...
    results.append(
        run_test(
            "Confirmar exclusão (GET id apagado)",
            "GET",
            f"{base}/api/books/{livro_id}",
            esperado=404,
        )
    ...
    results.append(
        run_test(
            "Rejeitar rating fora de 1-5 (POST)",
            "POST",
            f"{base}/api/books",
            body={"title": "A", "author": "B", "rating": 9},
            esperado=422,
        )
    )
    ...
    results.append(
        run_test(
            "Limpar livro de teste de ISBN (DELETE)",
            "DELETE",
            f"{base}/api/books/{id_isbn}",
            esperado=204,
        )
    )
    ...
    for i, r in enumerate(results, 1):
        linhas.append(
            f"| {i} | {r['teste']} | {r['hora']} | {r['esperado']} | {r['obtido']} | {r['duracao_ms']}ms | {r['resultado']} |"
        )
    ...
    for i, r in enumerate(results, 1):
        ...
        if r["request_body"]:
            ...
            linhas.append("```")
        linhas.append(
            f"- Status esperado: `{r['esperado']}` / obtido: `{r['obtido']}` ({r['duracao_ms']}ms)"
        )
