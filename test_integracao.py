...
def http_request(method, url, body=None):
    ...
    try:
        ...
        return exc.code, exc.read().decode("utf-8", errors="replace"), dict((exc.headers or {}).items())
...
def container_health(container):
    code, out = run(["docker", "inspect", container, "--format", "{{.State.Health.Status}}"])
...
def network_containers(network):
    code, out = run(["docker", "network", "inspect", network, "--format", "{{range .Containers}}{{.Name}} {{end}}"])
    if code != 0:
        return []
...
def registrar(nome, passou, esperado, obtido, detalhe):
    ...
    log("=" * 70)
    resultados.append({"nome": nome, "esperado": esperado, "obtido": obtido, "log": detalhe, "resultado": resultado})
...
def main():
    ...
    registrar("Rede 'edge' conecta frontend e backend", edge_ok,
              f"contém {frontend} e {backend}", " ".join(edge_containers),
              f"docker network inspect {rede_edge} --format ...\n{' '.join(edge_containers)}")

    data_containers = network_containers(rede_data)
    data_ok = backend in data_containers and database in data_containers and frontend not in data_containers
    registrar("Rede 'data' conecta backend e database (sem frontend)", data_ok,
              f"contém {backend} e {database}, NÃO contém {frontend}", " ".join(data_containers),
              f"docker network inspect {rede_data} --format ...\n{' '.join(data_containers)}")

    code, internal_out = run(["docker", "network", "inspect", rede_data, "--format", "{{.Internal}}"])
    internal_ok = internal_out.strip() == "true"
    registrar("Rede 'data' é interna (isolada, sem saída)", internal_ok,
              "true", internal_out.strip(),
    ...
    proxy_ok = codigo == 200 and "nginx" in server_header.lower() and '"database":"connected"' in corpo
    registrar("Proxy /api do frontend para o backend", proxy_ok,
              "200, header Server: nginx, database:connected", "ver log",
    ...
    ports_ok = (not port_backend.strip()) and (not port_database.strip()) and ("80" in port_frontend)
    registrar("Somente a porta do frontend está publicada no host", ports_ok,
    ...
    health_ok = h_frontend == "healthy" and h_backend == "healthy" and h_database == "healthy"
    registrar("Os 3 serviços estão 'healthy'", health_ok,
              "healthy nos 3", f"frontend={h_frontend} backend={h_backend} database={h_database}",
    ...
    # 4. Volume: existência
    # ------------------------------------------------------------------
    code, vol_info = run(["docker", "volume", "inspect", volume, "--format", "{{.Name}}: {{.Mountpoint}}"])
    vol_ok = code == 0
    registrar("Volume do MySQL existe", vol_ok,
              f"volume {volume} existe", vol_info,
    ...
    try:
        body_criar = {"title": marcador, "author": "Script de integracao", "category": "Teste", "reading_status": "LENDO"}
        ...
        persist_ok = ok_frontend and ok_backend and ok_database and (titulo_confirmado == marcador)
    except Exception as exc:
        log_persist.append(f"\nFalhou: {exc}")
        persist_ok = False

    registrar("Persistência do volume (cadastro sobrevive a docker compose restart)", persist_ok,
              f"livro '{marcador}' continua existindo e os 3 containers voltam healthy", "ver log",
              "\n".join(log_persist))
    ...
    for i, r in enumerate(resultados, 1):
        linhas.append(f"| {i} | {r['nome']} | {r['esperado']} | {r['obtido']} | {r['resultado']} |")
    ...
    linhas.append("")
    linhas.append("Saída de console completa e sem edição, na ordem em que as checagens rodaram.")
    linhas.append("")
    linhas.append("```text")
    ...
    if total_ok != total:
        print(f"{total - total_ok} checagem(ns) falharam - veja o relatório para detalhes.")
    print("=" * 70)
    print()
